  ...
  describe "POST #create" do 
    ...
    context "with invalid params" do 
      it "raises validation error with invalid params" do 
        post :create, params: { quantity: -1 }, format: :json
        parsed_response = JSON.parse(response.body)
        expect(response).to have_http_status(422)
        expect(parsed_response['message']).to match(/name/i)
      end
    end
  end
  ...
  describe "PUT #update" do 
    context "with valid params" do 
      ...
      it "updates the requested invoice option" do
        put :update, params: new_attributes.merge({ id: invoice_option.to_param }), format: :json
    ...
    context "with invalid params" do 
      ...
      it "doesn't update invoice option with invalid params" do 
        put :update, params: { quantity: -1 }.merge({ id: invoice_option.to_param }), format: :json
        parsed_response = JSON.parse(response.body)
        expect(response).to have_http_status(422)
