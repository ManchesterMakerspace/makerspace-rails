  ...
  describe "PUT #update" do 
    context "with valid params" do 
      ...
      it "updates the requested invoice option" do
        ...
        expect(invoice_option.due_date).to eq(Time.parse(new_attributes[:quantity].to_s))
      end
      ...
      it "renders json of the invoice_option" do
        ...
        expect(response).to have_http_status(200)
        expect(response.content_type).to eq "application/json"
        expect(parsed_response['invoice_option']['id']).to eq(invoice_option.id.as_json)
      end
    end
    ...
    context "with invalid params" do 
      ...
      it "doesn't update invoice option with invalid params" do 
        put :update, params: {invoice_option: {id: invoice_option.to_param, quantity: -1}}, format: :json
        parsed_response = JSON.parse(response.body)
        expect(response).to have_http_status(422)
        expect(parsed_response['message']).to match(/quantity/)
      end
