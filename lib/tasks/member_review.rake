...
task :member_review => :environment do
  ...
  @management_messages = ["--- Weekly Membership Review ---"]
  ...
  if should_run
    begin
      ...
      # Helpers
      def send_report(member_list, management_message, no_slack_message, slack_lambda)
        build_management_messages(management_message, member_list)
        slack_users = SlackUser.in(member_id: member_list.map(&:id))
        all_slack_user_ids = slack_users.map(&:member_id)
        non_slack_users = member_list.select { |member| !all_slack_user_ids.include?(member.id) }

        if (non_slack_users.length > 0)
          build_management_messages(no_slack_message, non_slack_users)
        end
      ...
      def build_management_messages(title, members)
        @management_messages.push(title)
        members.each { |m| @management_messages.push("<#{@base_url}/members/#{m.id}|#{m.fullname}>") }
      end
      ...
      def notify_orientaion_message
        send_report(
          ...
          "The following members do not have Slack but need orientation",
      ...
      def notify_no_purchase_message
        send_report(
          ...
          "The following users never accepted the Slack invite when creating an account",
          Proc.new do |slack_user| 
            notify_member(
      ...
      def notify_paypal_message
        send_report(
          ...
          "The following members do not have slack but are still on PayPal billing",
          Proc.new do |slack_user| 
            notify_member(
      ...
      def notify_missing_contracts(missing_contracts, contract_type)
        send_report(
          ...
          "These members do not have Slack but need to sign a #{contract_type}",
          Proc.new do |slack_user| 
            notify_member(
      ...
      ::Service::SlackConnector.send_slack_messages(@management_messages, ::Service::SlackConnector.members_relations_channel)
    rescue => e
      error = "#{e.message}\n#{e.backtrace.inspect}"
